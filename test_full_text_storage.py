...
def test_full_text_in_qdrant():
    ...
    try:
        ...
        try:
            ...
            print("  Run ingestion first: python ingest_pool.py --workers 4 --max-pdfs 5")
            return False
        ...
        for i, point in enumerate(points):
            ...
            if text_len <= 200:
                print(f"  Point {i}: Text is only {text_len} chars (might be short chunk or preview)")
            else:
                full_text_found += 1
            ...
            if 'text_preview' in payload:
                issues_found.append(f"Point {i}: Old 'text_preview' field still exists!")
...
def test_payload_structure():
    ...
    try:
        ...
        required_fields = ['doc_id', 'filename', 'source_type', 'department', 'sequence', 'text']
